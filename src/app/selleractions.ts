...
const sellerschema = z.object({
  ...
  accountNumber: z
    .string()
    .min(3, { message: "The name has to be a min charackter length of 5" }),
  // cnicImages: z.array(z.string(), { message: "Images are required" }),
  // checkbookImage: z.array(z.string(), { message: "Image is required" }),
});

export async function seller(prevState: any, formData: FormData) {
  ...
  const validateFields = sellerschema.safeParse({
    ...
    // cnicImages: JSON.parse(formData.get("cnicImages") as string),
    // checkbookImage: JSON.parse(formData.get("checkbookImage") as string),
  ...
  const data = await prisma.seller.create({
    data: {
      ...
      fatherName: validateFields.data.fatherName,
      email: validateFields.data.email,
      phone: validateFields.data.phone,
      cnic: validateFields.data.cnic,
      accountType: validateFields.data.accountType,
      accountNumber: validateFields.data.accountNumber,
      // cnicImages: validateFields.data.cnicImages,
      // checkbookImage: validateFields.data.checkbookImage,
      id: user.id,
    },
  });
